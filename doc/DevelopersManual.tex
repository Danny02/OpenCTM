...
Texture coordinates & ctmTexCoordPrecision()\\ \hline
...
mesh and what is going to be used for (for instance, if it will be used in a
measurment or manufacturing process). For example, if the vertex coordinate
unit is meters (which is the recommended unit for OpenCTM meshes), and the
precision is specified as 0.001, the fixed point precision will be 1 mm:
...
(magnitude). For instance, 0.01 means that the circle is divided into 100 steps,
and the normal magnitude is rounded to 2 decimals:
...
\subsection{Texture coordinate precision}
Texture coordinate precision is specified on a per texture map basis, and
gives the absolute precision in texture coordinate space.

The effects of different precisions depend on many different things, for
instance the resolution of the texture (a 4096x4096 texture may require
better precision than a 256x256 texture) and the resolution of the mesh.

To specify a resolution of 0.001 for the texture map $texMap$, use:

\begin{lstlisting}
  ctmTexCoordPrecision(context, texMap, 0.001);
\end{lstlisting}

The default texture coordinate precision is $2^{-12} \approx 0.00024$.


\subsection{Custom attribute precision}
As with texture coordinates, the precision for custom vertex attributes are
...
%The OpenCMT file format is very open and flexible, and different applications
%are free to use different conventions, for instance for units, coordinate
%systems, attribute naming, etc.
%
%However, in order to create files that are easily interchangeable between
%applications, some recommendations can be followed.
%
%The recommendations that are outlined in this chapter are by no means
%mandatory, but they can serve as a guide whenever there is an option for
%an application to select one of sveral conventions.
%
%
%\section{Triangle orientation}
...
%    \item \textellipsis $(u=0, v=0)$ represents the lower left corner of the
%          texture, and $(u=1, v=1)$ represents the upper right corner of the
%          texture.
%\end{itemize}
%
%
%\section{Custom vertex attributes}
%It is recommended (but not required) that:
%\begin{itemize}
